...
print("หากพบเจอบัคโปรดแจ้งที่ Discord")
print("STATS : Online")


local serv = win:Server("Funtion ( Premium )", "")
...
AutoFram:Toggle("[ ! ] Auto Farm",false,function(vu)
   if SelectToolWeapon == "" and vu then
      print("Selcet Weapond ก่อนทุกทั้ง")
   else
      local args = {
            [1] = "AbandonQuest"
      }
                           ...
                           if game:GetService("Workspace").Enemies:FindFirstChild(Monster) then
                              if string.find(LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMonster) then
                                 ...
                                 if v.Humanoid.Health <= HealthMin then
                                    ...
                                    v.HumanoidRootPart.CanCollide = false
                                    v.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,20,15)
                                    game:GetService'VirtualUser':CaptureController()
                                    game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))  
                                 end
...
AutoFram:Toggle("[ ! ] Auto Quest Bartolomeo ",false,function(v)
   ...
   elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 and Bool == true then
      print("Quest Bartolomeo เสร็จแล้ว")
   else
      AutoBartilo = v
   end
...
stat:Toggle("[ * ] Melee ( หมัด )",false,function(MELEE)
    shared.MELEE = MELEE
    end)
    spawn(function()
while wait() do
...
stat:Toggle("[ * ] Defense ( เลือด )",false,function(Defense)
...
stat:Toggle("[ * ] Sword ( ดาบ )",false,function(Sword)
    shared.Sword = Sword
    end)
    spawn(function()
while wait() do
...
stat:Toggle("[ * ] Gun ( ปืน )",false,function(Gun)
    shared.Gun = Gun
    end)
    spawn(function()
while wait() do
...
stat:Toggle("[ * ] Blox Fruit ( ผลไม้ปีศาจ )",false,function(DF)
    shared.DF = DF
    end)
    spawn(function()
while wait() do
...
if NewWorld then
   ...
   Teleport:Button("GraveIsland [ 1 ]",function()
      game.Players.localPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-5411.47607, 48.8234024, -721.272522)
   end)
   Teleport:Button("GraveIsland [ 2 ]",function()
      game.Players.localPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-6036.49219, 6.74596024, -1309.7937)
   end)
   Teleport:Button("Snow Mountain [ 1 ]",function()
      game.Players.localPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(511.825226, 401.765198, -5380.396)
   end)
   Teleport:Button("Snow Mountain [ 2 ]",function()
      game.Players.localPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(1329.58472, 429.765228, -5297.57422)
   end)
...
Players:Toggle("[ ! ] Kill Player ( Gun )",false,function(bool)
   ...
   while KillPlayer do wait()
      game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Players:FindFirstChild(SelectedKillPlayer).Character.HumanoidRootPart.CFrame * CFrame.new(30,30,30)
      game.Players:FindFirstChild(SelectedKillPlayer).Character.HumanoidRootPart.Size = Vector3.new(60,60,60)
      game:GetService'VirtualUser':CaptureController()
      game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
   end
...
Raids:Label("[ คำเตือน : Use in Dungeon Only , ใช้ในดันเท่านั้น  ]")
...
Misc:Toggle("[ ! ] Bring All Fruit",false,function(value)
   TeleportDF = value
   pcall(function()
...
Misc:Seperator()
      ...
      local BuyItem = serv:Channel("Store") 
      BuyItem:Seperator()
